...
{
   unicap_copy_format( &buffer->format, format );
   buffer->private_ = malloc( sizeof( unicap_data_buffer_private_t ) );
   sem_init( &buffer->private_->lock, 0, 1 );   
   buffer->private_->ref_count = 0;
   buffer->private_->free_func = init_data->free_func;
   buffer->private_->free_func_data = init_data->free_func_data;
   buffer->private_->ref_func = init_data->ref_func;
   buffer->private_->ref_func_data = init_data->ref_func_data;
   buffer->private_->unref_func = init_data->unref_func;
   buffer->private_->unref_func_data = init_data->unref_func_data;
}

void unicap_data_buffer_free( unicap_data_buffer_t *buffer )
{
   sem_wait( &buffer->private_->lock );
   if( buffer->private_->ref_count > 0 ){
      TRACE( "freeing a buffer with refcount = %d!!!\n", buffer->private_.refcount );
   }
   if( buffer->private_->free_func ){
      buffer->private_->free_func( buffer, buffer->private_->free_func_data );
   }
   
   sem_destroy( &buffer->private_->lock );
   if (buffer->data)
      free( buffer->data );
   free( buffer );
}
...
{
   sem_wait( &buffer->private_->lock );
   buffer->private_->ref_count++;
   sem_post( &buffer->private_->lock );
...
unicap_status_t unicap_data_buffer_unref( unicap_data_buffer_t *buffer )
{
   ...
   if( buffer->private_->ref_count > 0 ){
      buffer->private_->ref_count--;
      if (buffer->private_->unref_func){
	 buffer->private_->unref_func (buffer, buffer->private_->unref_func_data);
      }
      if (buffer->private_->ref_count == 0 ){
